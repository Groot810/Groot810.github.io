...
const modelServices = reactive<Record<ServiceKind, ModelServiceConfig>>({
  text: {
    providerName: 'OpenAI',
...
const connectionTests = reactive<Record<ServiceKind, ConnectionState>>({
...
const activeService = computed(() => modelServices[activeServiceKind.value])
const activeConnectionTest = computed(() => connectionTests[activeServiceKind.value])

function resetModelService(kind: ServiceKind = activeServiceKind.value) {
  const defaults: Record<ServiceKind, ModelServiceConfig> = {
    text: {
      providerName: 'OpenAI',
      baseUrl: 'https://api.openai.com/v1',
      apiKey: '',
  ...
  Object.assign(modelServices[kind], defaults[kind])
  connectionTests[kind].status = 'idle'
  connectionTests[kind].message = ''
...
function extractNodeContent(node: CanvasNode) {
  ...
  if (node.kind === 'config')
    return {
      ...base,
      prompt: node.content,
      model: modelServices.image.model,
...
function serviceForNode(node: CanvasNode) {
  return modelServices[node.kind === 'config' ? 'text' : node.kind]
}
...
async function testProviderConnection(kind: ServiceKind = activeServiceKind.value) {
  const service = modelServices[kind]
  const connectionTest = connectionTests[kind]
...
function writeImageResult(node: CanvasNode, imageUrl: string, prompt = '') {
  ...
  node.content = `由 ${modelServices.image.model} 生成 · ${new Date().toLocaleTimeString()}`
  node.version = (node.version || 0) + 1
}
...
async function callConfiguredImage(
  ...
  const service = modelServices.image
...
function createImageBatchResultNodes(
  ...
  imageResults.forEach((result, index) => {
    const child: CanvasNode = {
      ...
      resultText: `已通过 ${modelServices.image.model} 生成图像`,
    }
...
function createImageVariationResultNodes(
  ...
  return imageResults.map((result, index) => {
    const child: CanvasNode = {
      ...
      resultText: `已通过 ${modelServices.image.model} 修改图像`,
    }
    writeImageResult(child, result.imageUrl, result.prompt)
    nodes.value.push(child)
    edges.value.push({
...
async function runImageVariation(source: CanvasNode) {
  ...
  if (!modelServices.image.apiKey.trim()) return flash('请先配置图片服务 API Key')
...
function uploadAsset(event: Event) {
  ...
  reader.onload = () => {
    ...
    addNode(kind, 30)
    const node = nodes.value.at(-1)!
    node.title = file.name
    node.url = String(reader.result)
    node.content = `${(file.size / 1024 / 1024).toFixed(1)} MB · 本地资产`
  }
...
function saveNow(silent = false) {
  ...
  const publicModelServices = Object.fromEntries(
    serviceOptions.map(({ kind }) => [kind, { ...modelServices[kind], apiKey: '' }]),
  )
  const publicSettings = { ...settings, modelServices: publicModelServices }
  localStorage.setItem('infinite:settings', JSON.stringify(publicSettings))
  serviceOptions.forEach(({ kind }) => {
    const key = `infinite:api-key:${kind}`
    if (modelServices[kind].apiKey) sessionStorage.setItem(key, modelServices[kind].apiKey)
...
function clearLocalData() {
  ...
  canvasIndex.value = []
  sessionStorage.removeItem('infinite:api-key')
  serviceOptions.forEach(({ kind }) => sessionStorage.removeItem(`infinite:api-key:${kind}`))
  flash('本地数据已清理')
}
function seedCanvas() {
  nodes.value = [
...
function loadLocal() {
  ...
  if (parsedSettings) {
    ...
    if (savedModelServices) {
      serviceOptions.forEach(({ kind }) => {
        if (savedModelServices[kind]) Object.assign(modelServices[kind], savedModelServices[kind])
      })
    } else {
      const legacyBaseUrl =
      ...
      Object.assign(modelServices.text, {
        providerName: legacyProvider,
        baseUrl: legacyBaseUrl || modelServices.text.baseUrl,
        model:
          parsedSettings.model === 'gpt-5.2' || parsedSettings.model === 'gpt-5.6'
            ? 'gpt-5.5'
            : parsedSettings.model || modelServices.text.model,
      })
      Object.assign(modelServices.image, {
        providerName: legacyProvider,
        baseUrl: legacyBaseUrl || modelServices.image.baseUrl,
        model:
    ...
    serviceOptions.forEach(({ kind }) => {
      if (
        savedModelServices?.[kind]?.temperature == null &&
        typeof legacyTemperature === 'number'
      )
        modelServices[kind].temperature = legacyTemperature
      if (savedModelServices?.[kind]?.maxTokens == null && typeof legacyMaxTokens === 'number')
        modelServices[kind].maxTokens = legacyMaxTokens
    })
  }
  settings.theme = 'dark'
  const legacyApiKey = sessionStorage.getItem('infinite:api-key') || ''
  serviceOptions.forEach(({ kind }) => {
    modelServices[kind].apiKey =
      sessionStorage.getItem(`infinite:api-key:${kind}`) ||
      (kind === 'text' || kind === 'image' ? legacyApiKey : '')
  })
  loadCanvasIndex()
...
<template>
  <div class="app-shell">
    <header class="topbar">
      ...
      <button class="avatar">LY</button>
    </header>

    <main class="workspace">
      <aside class="left-rail">
        <button :class="{ active: mode === 'select' }" title="选择 (V)" @click="mode = 'select'">↖</button>
        <button :class="{ active: mode === 'hand' }" title="移动画布 (H)" @click="mode = 'hand'">✋</button>
        <div class="rail-divider"></div>
        <button v-for="item in toolbarItems" :key="item.kind" :title="item.label" @click="addNode(item.kind)">
          <b>{{ item.icon }}</b><span>{{ item.label }}</span>
        </button>
        <button title="上传资产" @click="fileInput?.click()">⇧<span>上传</span></button>
        <input ref="fileInput" hidden type="file" accept="image/*,video/*,audio/*" @change="uploadAsset" />
        ...
        <div class="canvas-transform" :style="{ transform: `translate(${viewport.x}px, ${viewport.y}px) scale(${viewport.zoom})` }">
          ...
          >
            ...
            <div v-else-if="node.kind === 'audio'" class="audio-preview">
              <button>▶</button><div class="wave">▂▅▃▇▄▆▂▅▇▃▆▄▂▇▅▃▆</div>
            </div>
            <NodePromptEditor
              v-else
            ...
            >
              ...
              <footer>
                ...
                <button class="primary" @click="runImageVariation(node)">生成修改结果</button>
              </footer>
            </div>
            ...
            <div class="node-foot">
              ...
              <button v-if="node.kind === 'image' || node.kind === 'video' || node.kind === 'text'" class="run-button" @click.stop="runNode(node)">
          ...
          <div class="settings-body">
            ...
            <div class="settings-content">
              <template v-if="activeSetting === '模型服务'">
                ...
                <label>API Key
                  <div class="password-field"><input v-model="activeService.apiKey" type="password" placeholder="sk-••••••••••••••••" /><span>仅保存当前类型</span></div>
                </label>
                ...
                <div class="security-note">🔒 四种服务的 API Key 相互独立，仅保存在当前浏览器会话中。节点运行时会按节点类型选择对应的地址、密钥和模型。</div>
                <button class="secondary" :disabled="activeConnectionTest.status === 'testing'" @click="testProviderConnection(activeServiceKind)">
                  {{ activeConnectionTest.status === 'testing' ? '测试中…' : `测试${serviceOptions.find((item) => item.kind === activeServiceKind)?.label}服务` }}
                </button>
                <div
                ...
                <div class="section-reset">
                  <button @click="resetModelService(activeServiceKind)">
                    恢复默认{{ serviceOptions.find((item) => item.kind === activeServiceKind)?.label }}配置
                  </button>
                </div>
              </template>
